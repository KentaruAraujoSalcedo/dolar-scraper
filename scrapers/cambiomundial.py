...
def _looks_blocked(html: str) -> bool:
    h = (html or "").lower()
    # patrones típicos de WAF / bloqueos
...
async def _get_with_retries(client: httpx.AsyncClient, url: str, tries=4) -> httpx.Response:
    ...
    for i in range(tries):
        try:
            ...
            # Reintentar en rate-limit / errores server
...
async def scrap_cambiomundial():
    ...
    headers = {
        ...
        # ayuda a algunos WAF a “parecer browser”
    ...
    try:
        async with httpx.AsyncClient(headers=headers, timeout=timeout, follow_redirects=True) as client:
            ...
            await _get_with_retries(client, url, tries=2)
        ...
        if compra is None or venta is None:
            return {
                ...
                "estado": "error",
                "error": (
                    f"No se pudo extraer compra/venta. "
                    f"ct={ct} blocked={_looks_blocked(html)} "
                    f"snippet={_snippet(html)}"
                ),
            }
