...
async def scrap_marketdollar():
    ...
    try:
        async with async_playwright() as p:
            ...
            compra_loc = page.locator("#display-compra").first
            venta_loc  = page.locator("#display-venta").first

            await compra_loc.wait_for(state="attached", timeout=20000)
            await venta_loc.wait_for(state="attached", timeout=20000)

            # Esperar a que tengan formato de tasa real
            await page.wait_for_function(
                """() => {
                    const c = document.querySelector('#display-compra')?.textContent || '';
                    const v = document.querySelector('#display-venta')?.textContent || '';
            ...
            compra_txt = _pick_rate(compra_raw)
            venta_txt  = _pick_rate(venta_raw)
